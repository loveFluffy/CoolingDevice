...
Public Class Form1
    ...
    Public Sub New()
        ...
        Try
            ...
            'Request the state of FIO6.
            LJUD.AddRequest(lngHandle, LJUD.IO.GET_DIGITAL_BIT, 6, 0, 0, 0)
            ...
            LJUD.GetResult(lngHandle, LJUD.IO.GET_DIGITAL_BIT, 6, dblFIO6)
            ...
            'Now a timer and counter example.  Connect a jumper on
            'the UE9 from FIO4 (PWM output) to FIO5 (Counter1 input).
            'Use the 1MHZ_DIV timer clock base, meaning that the clock
            'divisor option is available, but Counter0 is not available.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.TIMER_CLOCK_BASE, LJUD.TIMERCLOCKS.MHZ1_DIV, 0, 0)
            'Set the divisor to 2 so the actual timer clock is 500kHz.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.TIMER_CLOCK_DIVISOR, 2, 0, 0)
            'Enable 1 timer.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.NUMBER_TIMERS_ENABLED, 1, 0, 0)
            'Offset the timer/counters by 4 so they start appearing on FIO4.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.TIMER_COUNTER_PIN_OFFSET, 4, 0, 0)
            'Configure Timer0 as 8-bit PWM.  Frequency will be 500k/256 = 1954 Hz.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_TIMER_MODE, 0, 1, 0, 0)
            'Set the PWM duty cycle to 50%.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_TIMER_VALUE, 0, 32768, 0, 0)
            'Enable Counter1.  It will use FIO5.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_COUNTER_ENABLE, 1, 1, 0, 0)
            'Execute the requests on a single LabJack.  The driver will use a
            ...
            LJUD.GetResult(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.TIMER_CLOCK_BASE, 0)
            LJUD.GetResult(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.TIMER_CLOCK_DIVISOR, 0)
            LJUD.GetResult(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.NUMBER_TIMERS_ENABLED, 0)
            LJUD.GetResult(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.TIMER_COUNTER_PIN_OFFSET, 0)
            LJUD.GetResult(lngHandle, LJUD.IO.PUT_TIMER_MODE, 0, 0)
            LJUD.GetResult(lngHandle, LJUD.IO.PUT_TIMER_VALUE, 0, 0)
            LJUD.GetResult(lngHandle, LJUD.IO.PUT_COUNTER_ENABLE, 1, 0)

            'Wait half a second, then read Counter1.  Should get roughly 977 counts.
            Sleep(500)
            LJUD.eGet(lngHandle, LJUD.IO.GET_COUNTER, 1, dblCount1, 0)
            txtCount1.Text = Str(dblCount1)

            'Disable the timer and counter.
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_CONFIG, LJUD.CHANNEL.NUMBER_TIMERS_ENABLED, 0, 0, 0)
            LJUD.AddRequest(lngHandle, LJUD.IO.PUT_COUNTER_ENABLE, 1, 0, 0, 0)
            LJUD.GoOne(lngHandle)
        Catch x As LabJackUDException
            ErrorMessage(x)
        End Try
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.txtFIO6 = New System.Windows.Forms.TextBox
        ...
        'Label8
        '
        Me.Label8.Location = New System.Drawing.Point(116, 280)
        Me.Label8.Name = "Label8"
        Me.Label8.Size = New System.Drawing.Size(240, 64)
        Me.Label8.TabIndex = 38
        Me.Label8.Text = "Connect a jumper on the U3 from FIO4 (PWM output) to FIO5 (Counter1 input).  The " & _
        "program will then wait half a second, then read Counter0.  It should get roughly" & _
        " 488 counts"
        '
        'txtCount1
        '
        ...
        'txtFIO6
        '
        Me.txtFIO6.Location = New System.Drawing.Point(12, 264)
        Me.txtFIO6.Name = "txtFIO4"
        Me.txtFIO6.Size = New System.Drawing.Size(80, 20)
        Me.txtFIO6.TabIndex = 36
        Me.txtFIO6.Text = ""
        '
        'txtAIN1
        '
        ...
        Me.Name = "Form1"
        Me.Text = "Simple U3 VB.NET Example"
